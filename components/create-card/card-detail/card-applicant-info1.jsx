...
function CardApplicantInfo1() {
    ...
    const validateField = (field, value) => {
        ...
        switch (field) {
            case 'residenceNumber':
                if (value.length !== 6) {
                    newErrors[field] = '주민등록번호 앞자리 6자리를 입력해주세요.';
                } else {
            ...
            case 'residenceNumber2':
                if (value.length !== 7) {
                    newErrors[field] = '주민등록번호 뒷자리 7자리를 입력해주세요.';
                } else {
                    delete newErrors[field];
                }
                break;
    ...
    const handleSubmit = () => {
        // 모든 필드 검증
        const allFields = ['residenceNumber', 'residenceNumber2', 'name', 'englishName'];
        let isValid = true;
									...
									<div className="relative">
											...
											<div className="flex items-center space-x-4">
													...
													<input
															...
															className={`w-40 p-3 border-2 rounded-xl transition-all duration-300 outline-none
																	${touched.residenceNumber2 && errors.residenceNumber2 
																			? 'border-red-500 bg-red-50' 
																			: 'border-gray-200 bg-gray-50 focus:border-blue-500 focus:bg-white'
																	}`}
															placeholder="*******"
											...
											{(touched.residenceNumber && errors.residenceNumber) && (
													<div className="flex items-center mt-2 text-red-500">
															...
															<p className="text-sm">{errors.residenceNumber}</p>
													</div>
											)}
									</div>
