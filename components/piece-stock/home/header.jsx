...
const HomeHeader = ({ data, setValue, value ,setPage}) => {
    ...
    return (
        ...
        <div className='flex justify-between mb-10 mt-5'>
            ...
            <div className="bg-slate-200 p-10 rounded-2xl m-14">
                <div className="text-left">
                    <h1 className="font1 font-bold mb-3">내 투자 현황</h1>
                    <p className="font4 font-bold mb-3">{formatNumber(headDummyData.price)} 원</p>
                    <div className='flex'>
                        <p className={`font-bold ${headDummyData.rate > 0 ? `text-red-500` : `text-blue-500`}`}>{formatNumberByTotal(headDummyData.price*headDummyData.rate*0.01/(1+headDummyData.rate/100))}</p>
                        <p className={`font-bold ${headDummyData.rate > 0 ? `text-red-500` : `text-blue-500`}`}>{headDummyData.rate > 0 ? "(+" + headDummyData.rate + "%)" : "("+headDummyData.rate + "%)"}</p>
                    </div>
                </div>
            </div>
        </>
