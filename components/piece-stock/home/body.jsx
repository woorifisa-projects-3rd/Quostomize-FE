        ...
        <>
            <div className="p-4 h-screen flex flex-col">
                <div className="flex justify-between">
                    <div>
                        <button className={`p-3 font3 outline-none ${page[0] === true ? `border-b-4` : null} border-slate-300`} onClick={()=>setPage([true,false,false])}>보유</button>
                        <button className={`p-3 font3 outline-none ${page[1] === true ? `border-b-4` : null} border-slate-300`} onClick={()=>onFavorite(param,setWishInfo,setPage,cardId)}>관심</button>
                    </div>
                ...
                <div className="flex-grow overflow-auto mt-5">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span className="font1 text-gray-500 ml-1">{stock.stockNumber}주</span>
                                </div>
                            </div>
                            <div className="text-right flex flex-col">
                                <div className="font1 text-gray-500">{stock.stockPrice}원</div>
                                <div className="font-semibold flex items-center">
                                <p className={`font-bold ${stock.stockRate > 0 ? `text-red-500` : `text-blue-500`}`}>{stock.stockRate > 0 ? "+" + stock.stockRate + "%" : stock.stockRate + "%"}</p>
                                </div>
                            </div>
                        </div>
                    ))}
                    {page[1] && wishInfo !== undefined && wishInfo.map((wishStock, index)=>(
                            ...
                            onDrop={(e) => handleDrop(e, index, wishInfo,dragOverIndex,setOrderInfo,setWishInfo,setDragOverIndex, orderInfo, cardId)}
